...
Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	for (unsigned int i = 0; i < mesh->mNumVertices; ++i)
	{
		...
		// 处理纹理
		if (mesh->mTextureCoords[0]) // 检查网格是否有纹理坐标
		{
			glm::vec2 vec;
...
std::vector<MyTexture> Model::loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
{
	...
	for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
	{
		...
		if (skip) continue;
		MyTexture myTexture;
		myTexture.texture.Assign(directory + "/" + str.C_Str());
		myTexture.type = typeName;
		myTexture.path = str.C_Str();
		textures.push_back(myTexture);
		textures_loaded.push_back(myTexture);
	}
	return textures;
}
